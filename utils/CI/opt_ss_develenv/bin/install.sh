...
function createLink(){
   [ $# == "1"] && symLinkName=`basename "$1"` || symLinkName="$2"
   ! [ -L $symLinkName] && ln -s $1 $symLinkName
}
...
function configureLinks(){
   ...
   su $PROJECT_USER -c "cd $PROJECT_HOME && rm -Rf docs && ln -s /var/$APPNAME/docs docs"
   ...
   su $PROJECT_USER -c "mkdir -p $PROJECT_HOME/app/jenkins/"
   pushd . >/dev/null
   cd $PROJECT_HOME/app/jenkins
   for jenkinsConfFile in `find "/etc/$APPNAME/jenkins" -name "*.xml"`; do
      su $PROJECT_USER -c "cd $PROJECT_HOME/app/jenkins && rm -Rf `basename $jenkinsConfFile` && ln -s $jenkinsConfFile"
   done;
   popd >/dev/null
   ...
   su $PROJECT_USER -c "cd $PROJECT_HOME/app/repositories && rm -Rf storage-nexus && ln -s /var/$APPNAME/nexus/storage nexus"
   ...
   su $PROJECT_USER -c "mkdir -p /var/$APPNAME/temp && cd $PROJECT_HOME && rm -Rf /var/$APPNAME/temp && ln -s /var/$APPNAME/temp"
   su $PROJECT_USER -c "mkdir -p /var/tmp/$APPNAME && cd $PROJECT_HOME && rm -Rf temp && ln -s /var/tmp/$APPNAME temp"
   su $PROJECT_USER -c "cd $PROJECT_HOME/platform/tomcat/ && ln -s /var/tmp/$APPNAME/tomcat/temp"
   su $PROJECT_USER -c "cd $PROJECT_HOME/platform/tomcat/ && ln -s /var/tmp/$APPNAME/tomcat/work"
   # Apache
...
function installDevelenvPlugins(){
   ...
   ./installPlugin.sh python_plugin http://develenv-python-plugin.googlecode.com/files/python_plugin-17.dp
...
function getOrganizationConfigurationFile(){
   if [ "$organization" != "" ]; then
     ...
     curl $organization  > /opt/${ORG_ACRONYM}/$APPNAME/install/conf/default_organization.properties
     if [ "$?" != 0 ]; then
...
function areParametersOk(){
   if [ "$administratorId" == "" ]; then
      echo "\\nFalta definir el administrador de $PROJECT_NAME"
   fi
   if [ "$adminPassword" == "" ]; then
      echo "\\nFalta el password para el administrador de $PROJECT_NAME"
   fi
}
...
Revisa $INSTALL_DIR_LOG/${PROJECT_NAME}.error.log y $INSTALL_DIR_LOG/${PROJECT_NAME}.log"
   ...
   if [ "$?" != "0" ]; then
      _log "[ERROR] Error durante la personalización.
Revisa $INSTALL_DIR_LOG/${PROJECT_NAME}.error.1.log y $INSTALL_DIR_LOG/${PROJECT_NAME}.log"
      exit 1
   fi
   ...
   . ./setEnv.sh
   ./installSonarDB.sh
   chown -R develenv:develenv /var/tmp/rpm/
   su $PROJECT_USER -c "./develenv-repo.sh"
   chattr +i $PROJECT_HOME/app/repositories/rpms/noarch/ss-develenv-repo-*.rpm
   rm develenv-repo.sh
   rm -Rf /etc/yum.repos.d/${ORG_ACRONYM}-$APPNAME-*.repo
...
sed -i s:"^ADMINISTRATOR_ID=.*":"ADMINISTRATOR_ID=$administratorId":g $DIR/setEnv.sh
. $DIR/setEnv.sh
if [ "$okParameters" != "" ]; then
   if [ "$rpmPhase" == "" ] && ! [ -f /etc/$APPNAME/${APPNAME}.properties ]; then
